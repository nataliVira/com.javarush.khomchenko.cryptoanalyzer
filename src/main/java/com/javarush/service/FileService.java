...
public class FileService {
    ...
    public static List<Path> process(UserInfo userInfo) throws IOException, ProcessException {
        ...
        if (userInfo.getTypeOperation() == TypeOperation.CRYPTANALYSIS) {
            for (int i = 1; i < Alphabet.size(); i++) {
                ...
                files.add(processFile(newUserInfo));
            }
        } else {
            files.add(processFile(userInfo));
        }
        return files;
    }

    private static Path processFile(UserInfo userInfo) throws IOException, ProcessException {
        checkFile(userInfo);
